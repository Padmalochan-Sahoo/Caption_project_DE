...
#sc = SparkContext(appName='dz_test')
#sc.setLogLevel('WARN')
...
dfCSV=spark.readStream.format('kafka').option('kafka.bootstrap.servers','kafka:9092').option('subscribe', 'dz_data_csv').option("failOnDataLoss","false").option('startingOffsets', 'earliest').load().selectExpr("CAST(value AS STRING)")
dfCSV.printSchema()
...
'Country_code_final',
'Country_name',
'Country_total_deaths','Extracted_timestamp')).alias('value')).writeStream.format("kafka").outputMode("complete").option("failOnDataLoss","false").option('checkpointLocation','/checkpoint/').option("kafka.bootstrap.servers","kafka:9092").option("topic", "dz_out").start().awaitTermination()
